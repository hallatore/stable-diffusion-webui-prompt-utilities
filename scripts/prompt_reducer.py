...
class Script(scripts.Script):
    def title(self):
        return "Prompt reducer"

    def ui(self, is_img2img):
        info = gr.Label("This script will generate a grid of images with different parts of the prompt removed. This can be used to find out what each part does to the image.")
        positives = gr.Checkbox(value=True, label="Include positive prompt")
        skip_first_positives = gr.Number(value=1, label="Skip first N parts of positive prompt")
        ...
        return [info, positives, skip_first_positives, negatives, skip_first_negatives]

    def run(self, p, info, positives, skip_first_positives, negatives, skip_first_negatives):
        ...
        if negatives:
            for negative_prompt in negative_prompts:
                copy_p = copy.copy(p)
                copy_p.negative_prompt = negative_prompt[1]
